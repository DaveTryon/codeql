...
from DOM::ElementDefinition e
  ...
  and not exists(DefinedRole role | role.isSupportedRole(e.getAttributeByName("role")) = true)
select unquotedString(e.getAttributeByName("role").getValueNode().toString()), e

// Analog of aria-required-attr (debugged and working, but ignores focus consideration and closed comboboxes)
// from DOM::ElementDefinition e
//   where exists(e.getAttributeByName("role"))
//   and exists(DefinedRole role | (
